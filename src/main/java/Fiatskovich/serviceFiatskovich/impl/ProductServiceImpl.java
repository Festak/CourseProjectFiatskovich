...
public class ProductServiceImpl extends TemplateServiceImpl implements ProductService {
    ...
    @Override
    @Transactional
    public void setRatingToProduct(long id, int rating){
        Product product = productDao.findOne(id);
    ...
    public void addProductViewModelToDataBase(ProductViewModel product) {
        ...
        observerImp.notifyObservers(getCategoriesByName(productToDatabase));
    }

    @Transactional
    private Set<Category> getCategoriesByName(Product product) {
        Set<Category> set = new LinkedHashSet<>();
        Set<Category> daoSet = new LinkedHashSet<>(categoryDao.findAll());
        for (Category category : product.getCategories()) {
            for (Category daoCategory : daoSet) {
                if (category.getName() == daoCategory.getName()) {
                    set.add(daoCategory);
                }
            }
        }
